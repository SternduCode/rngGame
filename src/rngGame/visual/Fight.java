...
public class Fight extends Pane {
	...
	public void removeMobRan() {
		gamepanel.getMobRans().remove(mob);
	...
	public void scaleF11() {
		fight.setImage(ImgUtil.getScaledImage(gamepanel.getWindowDataHolder(), "./res/fight/Auswahl.png"));
		battlebackgroundvisual.setImage(ImgUtil.getScaledImage(gamepanel.getWindowDataHolder(), "./res/fight/Fight.png", gamepanel.getGameWidth(),
				gamepanel.getGameHeight()));
	...
	public void update() {
		...
		h.update();
		hh.update();
		if (demonMob.getCurrenthp() <= 0 && !f) {
			demonDead();
			f = true;
		}
		if (eigenMob.getCurrenthp() <= 0) for (int i = 0; i <= demonArray.length; i++) if (eigenMob.getCurrenthp() != 0) eigenMob = demonArray[i];
		else {
